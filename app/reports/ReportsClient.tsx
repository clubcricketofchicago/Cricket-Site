...
export default function ReportsClient({ initialReports }: { initialReports: MatchReport[] | null }) {
  const [reports, setReports] = useState<MatchReport[]>(initialReports ?? [])
  const [loading, setLoading] = useState<boolean>(initialReports === null)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    // The server page passes the reports when the DB answered; fetch only as
    // the fallback for that first-paint miss.
    if (initialReports !== null) return
